...
class MacAddress
{
    ...
    public static function setFakeMacAddress($interface, $mac = null)
    {
        ...
        $this->runCommand("ifconfig {$interface} down");
        $this->runCommand("ifconfig {$interface} hw ether {$mac}");
        $this->runCommand("ifconfig {$interface} up");

        // TODO: figure out if there is a better method of doing this
        // run DHCP client to grab a new IP address
        $this->runCommand("dhclient {$interface}");

        // run a test to see if the operation was a success
        if ($this->getCurrentMacAddress($interface) == $mac) {
            return true;
        }
